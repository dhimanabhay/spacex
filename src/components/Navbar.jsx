...
export default function Navbar() {
  ...
  return (
    <nav id="navbar" className=" flex flex-col fixed w-full ">
      <div
        className={` transition duration:600 flex flex-row justify-between h-[60px] items-center px-10 bg-black text-white font-nunito ${
        ...
        <div className="flex items-center gap-6 uppercase text-xs">
          {link_data.map((link) => (
            <div
              ...
              key={link.name}
            >
              <a href={link.link} className="hover:cursor-pointer hover:border-b hover:border-white border-l-0 border-r-0 border-t-0 border-transparent inline-block">
                {link.name}
              </a>
            </div>
          ))}
